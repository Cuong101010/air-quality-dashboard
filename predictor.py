...
def predict(recent_records):
    ...
    try:
        ...
        result = {
            ...
            'predictions': {
                '30min': {
                    'pm25': max(0.0, round(float(pred_30_actual[0]), 1)),
                    'temperature': round(float(pred_30_actual[1]), 1),
                    'humidity': max(0.0, min(100.0, round(float(pred_30_actual[2]), 1))),
                    'pressure': round(float(pred_30_actual[3]), 1),
                    'uv': max(0.0, round(float(pred_30_actual[4]), 2)),
                    'weather': weather_30,
                    'weather_vi': WEATHER_VI.get(weather_30, weather_30),
                    'time': ts_30.strftime('%H:%M'),
                },
                '60min': {
                    'pm25': max(0.0, round(float(pred_60_actual[0]), 1)),
                    'temperature': round(float(pred_60_actual[1]), 1),
                    'humidity': max(0.0, min(100.0, round(float(pred_60_actual[2]), 1))),
                    'pressure': round(float(pred_60_actual[3]), 1),
                    'uv': max(0.0, round(float(pred_60_actual[4]), 2)),
                    'weather': weather_60,
